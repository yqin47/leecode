...
class Twitter(object):
    ...
    def __init__(self):
        ...
        """
        self._number_of_most_recent_tweets=10
    ...
    def getNewsFeed(self, userId):
        ...
        for uid in self._followings[userId]:
            if self._messages[uid]:
                heapq.heappush(max_heap,(-self._messages[uid][-1][0],userId,0))
        result=[]
        while max_heap and len(result)<self._number_of_most_recent_tweets:
            t, uid, curr = heapq.heappop(max_heap)
            nxt = curr+1
            if nxt !=len(self._messages[uid]):
                heapq.heappush(max_heap,(-self._messages[uid][-(nxt+1)][0],uid,nxt))
            result.append(self._messages[uid][-(curr+1)][1])
        return result
